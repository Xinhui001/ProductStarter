<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 10px">
      ...
      <div>
        <el-table :data="data.tableData" style="width: 100%">
          <el-table-column prop="majorId" label="序号" width="70" />
    ...
    <el-dialog width="35%" v-model="data.formVisible" title="专业信息">
...
const save = () => {
  request.request({
        url: data.form.majorId? baseUrl+'/update' : baseUrl+'/add',
        method: data.form.majorId? 'POST' : 'POST',
        data: data.form
      }
  ).then(res =>{
    if(res.code === 0){
